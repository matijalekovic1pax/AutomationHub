...
def resolve_database_url() -> str:
    """Choose a DB URL that works locally and on read-only platforms like Vercel."""
    env_url = os.getenv("DATABASE_URL")
    if env_url:
        return env_url
    if os.getenv("VERCEL"):
        # Vercel serverless functions can only write under /tmp
        return "sqlite:////tmp/revithub.db"
    return "sqlite:///./revithub.db"
...
app = FastAPI(title="Automation Hub API", version="3.0.0", lifespan=lifespan)
...
if __name__ == "__main__":
    import uvicorn
    print("\n🚀 Starting Automation Hub Backend v3.0...")
