...
export const promoteUser = async (id: string | number): Promise<User> => {
  return await apiClient.post(`/users/${id}/promote`);
};

export const demoteUser = async (id: string | number): Promise<User> => {
  return await apiClient.post(`/users/${id}/demote`);
};
